...
@interface MagCalibrationMenuViewController ()

@end

@implementation MagCalibrationMenuViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    mag_calibration_menu_item_t item = mag_calibration_menu_item_calibrate;
    switch (rowIdx) {
        case 0:
            item = mag_calibration_menu_item_calibrate;
            break;
        case 1:
            item = mag_calibration_menu_item_state;
            break;
    }
    
    NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:item], kMagCalibrationMenuKeyItem, nil];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:kNoticationDidSelectMagCalibrationMenuItem
                                                        object:self
                                                      userInfo:userInfo];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
	NSString *cellIdentifier = @"MagcalibrationCellIdentifier";
